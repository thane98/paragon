...
from paragon.ui.views.ui_message_multi_line_widget import Ui_MessageMultiLineWidget


class MessageMultiLineWidget(AbstractAutoWidget, Ui_MessageMultiLineWidget):
    def __init__(self, state, field_id):
        AbstractAutoWidget.__init__(self, state)
        Ui_MessageMultiLineWidget.__init__(self)
    ...
    def refresh_value(self):
        ...
        if not self.current_path or not self.rid or not key:
            self.value.clear()
            return
        text = self.data.message(self.current_path, self.localized, key)
        text = text.replace("\\n", "\n") if text else ""
