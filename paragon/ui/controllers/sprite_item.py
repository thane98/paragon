        ...
        if self.sprite:
            ...
            if animation_data := self.sprite.animation_data:
                ...
                if self.animation_index < len(
                    animation_data
                ) and self.frame_index < len(
                    animation_data[self.animation_index].frame_data
                ):
                    return (
                        animation_data[self.animation_index]
...
class SceneSpriteItem(AbstractSpriteItem, QGraphicsItem):
    ...
    def get_current_frame_delay(self) -> int:
        return (
            1000
            / 60
            * self.animation_data["animation_data"][self.frame_index]["frame_delay"]
        )
    ...
    def next_frame(self):
        ...
        self.update(0, 0, self.sprite.width(), self.sprite.height())

    def boundingRect(self) -> QRectF:
        return QRectF(0, 0, self.sprite.width(), self.sprite.height())
