...
class FE10Maps:
    ...
    def get_terrain_colors(self, chapter_data: GcnChapterData):
        tile_rid, tile_field = self.gd.table("tiles")
    ...
    def tile_name(self, chapter_data: GcnChapterData, row, col) -> Optional[str]:
        if row < chapter_data.map_data.height and col < chapter_data.map_data.width:
            ...
            if tile_id:
                tile_rid = self.gd.key_to_rid("tiles", tile_id)
                if tile_rid:
    ...
    def load(self, zmap: str) -> GcnChapterData:
        ...
        dispos_c_path = f"zmap/{zmap}/dispos_c.bin"
        dispos_n_path = f"zmap/{zmap}/dispos_n.bin"
        dispos_h_path = f"zmap/{zmap}/dispos_h.bin"
        dispos_m_path = f"zmap/{zmap}/dispos_m.bin"
        dispos: GcnDisposData = GcnDisposData(
            common=utils.try_multi_open(self.gd, "dispos", dispos_c_path),
            normal=utils.try_multi_open(self.gd, "dispos", dispos_n_path),
            hard=utils.try_multi_open(self.gd, "dispos", dispos_h_path),
            maniac=utils.try_multi_open(self.gd, "dispos", dispos_m_path),
        )
