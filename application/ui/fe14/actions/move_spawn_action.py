...
class MoveSpawnAction:
    ...
    def undo(self):
        self._move_unit(self.old_position)

    def redo(self):
        self._move_unit(self.new_position)

    def _move_unit(self, position):
        ...
        message = "Updated position of %s to (%d, %d)." \
                  % (self.spawn.get_display_name(), position[0], position[1])
