...
fn to_record(types: &mut Types, spec: &AssetSpec) -> anyhow::Result<Record> {
    let mut record = types
        ...
        .ok_or_else(|| anyhow!("Type 'AssetSpec' does not exist."))?;
    record
        .field_mut("name")
    ...
    record
        ...
        .ok_or_else(|| anyhow!("Missing field."))?
        .set_bytes(spec.hair_color.to_vec())?;
    record
    ...
    record
        ...
        .ok_or_else(|| anyhow!("Missing field."))?
        .set_bytes(spec.skin_color.to_vec())?;
    record
    ...
    record
        ...
        .ok_or_else(|| anyhow!("Missing field."))?
        .set_bytes(spec.weapon_trail_color.to_vec())?;
    record
    ...
    record
        .field_mut("bitflags")
        .ok_or_else(|| anyhow!("Missing field."))?
        .set_bytes(spec.bitflags.to_vec())?;
    record
...
impl AssetStore {
    ...
    pub fn read(
        ...
        let mut table = types
            ...
            .ok_or_else(|| anyhow!("Type 'AssetTable' does not exist."))?;
        table
            .field_mut("flags")
            .ok_or_else(|| anyhow!("AssetTable is missing 'flags' field."))?
    ...
    pub fn write(&self, types: &Types, fs: &LayeredFilesystem) -> anyhow::Result<()> {
        match self.rid {
            Some(rid) => {
                ...
                binary.flags = types
                    .int(rid, "flags")
                    .ok_or_else(|| anyhow!("AssetTable has no 'flags' field."))?
                    as u32;
                let specs = types
                    .items(rid, "specs")
                    .ok_or_else(|| anyhow!("AssetTable has no 'specs' field."))?;
                for rid in specs {
                    let instance = types
                        .instance(rid)
